...
class SlaterKosterTable:
    ...
    def get_range(self, fractional_limit):
        ...
        for el in self.elements:
            ...
            print('Wave function range for %s = %.5f a0' % (el.get_symbol(), r), file=self.txt)
    ...
    def run(self, rmin=0.4, dr=0.02, N=None, ntheta=150, nr=50, wflimit=1e-7,
        ...
        print('\n\n', file=self.txt)
        print('***********************************************', file=self.txt)
        print('Slater-Koster table construction for %s and %s' %  (self.ela.get_symbol(), self.elb.get_symbol()), file=self.txt)
        print('***********************************************', file=self.txt)
        self.txt.flush()
        ...
        for i, R in enumerate(Rgrid):
            ...
            if  i == Nsub - 1 or Nsub // 10 == 0 or i % (Nsub // 10) == 0:
                print('R=%8.2f, %i grid points ...' % (R, len(grid)), file=self.txt, flush=True)
        ...
        if superposition == 'potential':
            ...
            print('     Relative error=%.2g %%' % (dH / Hmax * 100), file=self.txt)
